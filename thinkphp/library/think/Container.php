...
class Container implements ArrayAccess, IteratorAggregate, Countable
{
    ...
    public function invokeMethod($method, $vars = [])
    {
        try {
            ...
            $return = $reflect->invokeArgs(isset($class) ? $class : null, $args);
            if (is_callable([$class, 'finish']) && (empty($return) || is_bool($return) || isset($class->fetch))) {
                return $class->finish();
            } else {
                return $return ;
            }
    ...
    public function invokeReflectMethod($instance, $reflect, $vars = [])
    {
        ...
        $return = $reflect->invokeArgs($instance, $args);
        if (is_callable([$instance, 'finish']) && (empty($return) || is_bool($return) || isset($instance->fetch))) {
